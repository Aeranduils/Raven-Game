...
//change the friend state of the bot
void Raven_Bot::beFriend()
{
	m_bFriend = !m_bFriend;
}

//get the friend state of the bot
bool Raven_Bot::getFriend()
{
	return m_bFriend;
};
...
void Raven_Bot::Update()
{
	...
	if (!isPossessed())
	{
		...
		if (m_pTargSys->isTargetPresent())
		{
			...
			m_vecObservation.push_back(
				m_pWeaponSys->GetAmmoRemainingForWeapon(m_pWeaponSys->GetCurrentWeapon()->GetType()));
	...
	*/
	...
	//if bot from team, can collect weapons
	if	(isPossessed()||getFriend())
	{
		if (isAtPosition(m_pWorld->GetHQposition()))
		{
			m_pWeaponSys->CollectWeapon(this->ID());
		}
	}
}
