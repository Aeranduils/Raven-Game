...
void Raven_Game::TrainThread()
{
	...
	m_ModeleApprentissage = CNeuralNet(m_TrainingSet.GetInputNb(), m_TrainingSet.GetTargetsNb(), NUM_HIDDEN_NEURONS,
	                                   LEARNING_RATE);
...
void Raven_Game::Update()
{
	...
	if ((m_TrainingSet.GetInputSet().size() >= 1000) & (!m_LancerApprentissage))
	{
...
void Raven_Game::ClickRightMouseButton(POINTS p)
{
	...
	if (pBot && pBot == m_pSelectedBot)
	{
		...
		m_pSelectedBot->GetTargetSys()->ClearTarget();
	}
...
void Raven_Game::ClickMiddleMouseButton(POINTS p)
{
	if (m_pSelectedBot && m_pSelectedBot->isPossessed())
	{
		...
		if (pBot != nullptr)
		{
			//If T is pressed, add a new friend bot
			//else, add a new target and set new target for every friend
			if (IS_KEY_PRESSED('T'))
			{
				pBot->beFriend();
				if (pBot->getFriend())
				{
					if (m_pSelectedBot->GetTargetSys()->GetTarget() == pBot)
					{
						m_pSelectedBot->GetTargetSys()->ClearTarget();
					}
					pBot->GetTargetSys()->SetTarget(m_pSelectedBot->GetTargetSys()->GetTarget());
					debug_con << "Bot " << pBot->ID() << " befriended" << "";
				}
				else
				{
					pBot->GetTargetSys()->ClearTarget();
					m_pSelectedBot->GetBrain()->RemoveAllSubgoals();
					debug_con << "Bot " << pBot->ID() << " unbefriended" << "";
				}
			}
			else
			{
				if (m_pSelectedBot->GetTargetSys()->GetTarget() != pBot)
				{
					...
					for (Raven_Bot*& i : m_Bots)
					{
						if (i->getFriend())
						{
							i->GetTargetSys()->SetTarget(pBot);
						}
					}
					debug_con << "Bot " << pBot->ID() << " targeted" << "";
				}
				else
				{
					m_pSelectedBot->GetTargetSys()->ClearTarget();
					...
					for (Raven_Bot*& i : m_Bots)
					{
						if (i->getFriend())
						{
							i->GetTargetSys()->ClearTarget();
							i->GetBrain()->RemoveAllSubgoals();
						}
					}
					debug_con << "Bot " << pBot->ID() << " untargeted" << "";
				}
			}
		}
		else
		{
			//If H pressed, set headquarters potision to leave weapons
			if (IS_KEY_PRESSED('W'))
			{
				HQposition = POINTStoVector(p);

				debug_con << "HQ position : " << HQposition << "";
			}
			else
			{
				...
				for (Raven_Bot*& i : m_Bots)
				{
					if (i->getFriend())
					{
						i->GetTargetSys()->SetTarget(nullptr);
					}
				}
				debug_con << "No bot targeted" << "";
			}
		}
	}
}
...
//---------------------- GetHQposition ---------------------------------
//-----------------------------------------------------------------------------
Vector2D Raven_Game::GetHQposition()
{
	return HQposition;
}
...
{
	if (m_pSelectedBot && m_pSelectedBot->isPossessed() && m_pSelectedBot->GetTargetSys()->GetTarget() == nullptr)
	{
...
bool Raven_Game::isSecondVisibleToFirst(const Raven_Bot* pFirst,
...
void Raven_Game::Render()
{
	...
	if (m_pSelectedBot)
	{
		...
		if (m_pSelectedBot->GetTargetSys()->GetTarget() != nullptr)
		{
			gdi->RedPen();
			gdi->HollowBrush();
			gdi->Cross(m_pSelectedBot->GetTargetSys()->GetTarget()->Pos(),
			           m_pSelectedBot->GetTargetSys()->GetTarget()->BRadius() + 1);
		}
	...
	if (!GetHQposition().isZero())
	{
		gdi->GreenPen();
		gdi->HollowBrush();
		gdi->Cross(GetHQposition(), 10);
	}
}
